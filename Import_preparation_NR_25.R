...
beach <- import(here("Datasets", "Niagara", "2025-beach-Niagara.xlsx"))
follow <- import(here("Datasets", "Niagara", "2025-follow-Niagara.xlsx"))
...
investigate |> export(here("Datasets", "Niagara", "investigate.csv"))
...
survey_data$participant_id <- paste("NR_2025", survey_data$participant_id, sep = "_")

survey_data <- survey_data |> 
  relocate(row_id, .after = participant_id)

data_NR <- subset(survey_data, select = -c(email.x, email.y, phone))

# Create location-recruitment date variable

data_NR <- data_NR |> 
  mutate(recruit_date = paste("NR", date, sep = "_")) |> 
...
data_NR |> export(here("Datasets", "Niagara", "data_NR.csv"))
